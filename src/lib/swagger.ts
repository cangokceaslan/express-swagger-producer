...
import * as swaggerHelpers from '@lib/swagger-helpers';
import * as doctrineFile from 'doctrine-file';
 ...
 * @requires doctrine
 */
export function parseApiFile(file: any): any {
    const content = fs.readFileSync(file, 'utf-8');

    let comments: any = doctrineFile.parseFileContent(content, { unwrap: true, sloppy: true, tags: null, recoverable: true });
    return comments;
}
...
function parseReturn(tags: any) {
    let rets: any = {}
    let headers: any = parseHeaders(tags)
    ...
    for (let i = 1; i < tags.length; i++) {
        if (tags[i].title == 'property') {
            ...
            if (schema) {
                ...
                if (prop.type == 'enum') {
                    let parsedEnum: any = parseEnums('-eg:' + example)
...
function parseSecurity(comments: any) {
    let security;
    try {
...
function parseHeaders(comments: any) {
    let headers: any = {}
...
function parseEnums(description: any): any {
...
export function fileFormat(comments: any) {

    let route: any, parameters: any = {}, params: any = [], tags: any = [], definitions: any = {};
    for (let i in comments) {
                ...
                if (title == 'param') {
                    ...
                    if (!schema) {
                        ...
                        if (properties.type == 'enum') {
                            let parsedEnum: any = parseEnums(comments[i][j]['description'])
...
function filterJsDocComments(jsDocComments: any) {
    return jsDocComments.filter(function (item: any) {
 ...
 * @requires glob
 */
function convertGlobPaths(base: any, globs: any) {
    return globs.reduce(function (acc: any, globString: any) {
 ...
 */
export function generateSpecAndMount(app: any): any {
    ...
    return function (options: any) {
        ...
        parser.parse(swaggerObject, function (err: any, api: any) {
            if (!err) {
                swaggerObject = api;
            }
        });
        ...
        app.use(docs, function (req: any, res: any) {
